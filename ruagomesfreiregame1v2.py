...
print("Init [30,40,109] Goal [61,60,71]")
SP = SearchProblem(goal = [58,113,28], model = U, auxheur=coords)
tinit = time.process_time()
I = [98,71,25]
nn = SP.search(I,limitexp = 3000, limitdepth = 10, tickets = [40,13,8], anyorder = True)
tend = time.process_time()
print("%.1fms"%((tend-tinit)*1000))
if validatepath(nn,I,U, tickets = [40,13,8]):
        print("path")
        print(nn)
        plotpath(nn,coords)
else:
...
# print("Init [30,40,109] Goal [61,60,71]")
# SP = SearchProblem(goal = [6, 7, 8] , model = U, auxheur=coords)
# tinit = time.process_time()
# I =[1, 2, 3] 
